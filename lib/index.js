...
export default class PhoneInput extends Component {
  ...
  updateFlagAndFormatNumber(number, actionAfterSetState = null) {
    ...
    const prevIso2 = '' + this.state.iso2
    ...
    if (number) {
      ...
      const dialCode = PhoneNumber.getCountryDataByCode(prevIso2).dialCode
      if (phoneNumber.length <= dialCode.length + 1) {
        if (phoneNumber.slice(0, dialCode.length) !== '+' + dialCode) {
  ...
  render() {
    ...
    return (
      <View style={[styles.container, this.props.style]}>
        ...
        <View style={{ flex: 1, textAlignVertical: 'center', marginLeft: this.props.offset || 10 }}>
          <TextComponent
            ref={ref => {
              this.inputPhone = ref;
            }}
